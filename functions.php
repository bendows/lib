...
function bgoto ($a) {
 header("Location: http://".$_SERVER['HTTP_HOST']."/$a"); exit (0);
}
...
function posted () {
 ...
 function loggedin ()  {
   ...
   if (! isset($_SESSION['euid'])) { return (boolean) false; }
   if (empty($_SESSION['euid'])) { return (boolean) false; }
   if (! $_SESSION['euid']) { return (boolean) false; }
...
function selectbox_list($listboxname, $listboxdescription, $arr, $selectedkey)  {
 ...
 foreach ($arr as $key=>$val) {
   if ($arr[$key]==$selectedkey) { echo "<option selected value='".$arr[$key]."'>".$arr[$key]."</option>";}
   else  { echo "<option value='".$arr[$key]."'>".$arr[$key]."</option>";  }
 }
 echo "</select>";
}
...
function optionarray($arr, $selectedkey)  {
 foreach ($arr as $key=>$val) {
   if ((int)$key==(int)$selectedkey)  { echo "<option selected value='$key'>".$arr[$key]."</option>";}
   else  { echo "<option value='".$key."'>".$arr[$key]."</option>";  }
 }
}
...
function edittbox_list($arr)  {
 foreach ($arr as $key=>$val) {  echo "<input type='text' name='".$arr[$key]."'>"; }
}
...
preg_match("/(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)/", $src[$key]);
                       ...
                       case "zstr":
                               ...
                               (boolean) $check = preg_match ("/^[@\_A-Za-z0-9\/\!\ \.\,\-]*$/",
                               $src[$key]);
...
function striptags(&$value) {
 ...
 function sendemail (&$amsg=NULL) {
   ...
   if (! is_array($amsg)) { return (string) "no message"; }
   ...
   foreach ($amsg as $line) { $emsg[]=$line."\n"; }

   $estr=implode ('', $emsg); $l = (int) strlen($estr);

   $fp=popen ("$smbin -t","w");

   if (! $fp) { return "no pipe for ($smbin)"; }

   $r = (int) fwrite($fp,$estr); pclose($fp);
...
function phpemail ($fromname, $fromemail, $toname, $toemail, $subject, $amsg) {
 ...
 foreach ($amsg as $line) { $body[]=$line."\n"; }
 ...
 $body=implode ('', $body); $headers=implode ('', $headers);
...
function benemail ($fromname, $fromemail, $toname, $toemail, $subject, $msg) {
       ...
       $from = "From: $fromname <$fromemail>"; array_unshift ($msg,$from);
       $to = "To: $toemail"; array_unshift ($msg,$to);

   $subject = "Subject: $subject"; array_unshift ($msg, $subject);
...
function replaceimages ($afilearr=array())  {
    ...
    foreach ($afilearr as $key=>&$file) {
      ...
      if ( (int) $file['error'] !== 0) continue;
      if ( (string) $file['name'] === '') continue;
      if ( (string) $file['tmp_name'] === '') continue;
...
function resizeimage ($prefix, $subdir, $filename, $resizearray=array()) {
       ...
       if (! file_exists ($srcfilename)) {
          return "$srcfilename does not exist or is not a file"; }
					...
					$idx =array_keys($resizearray); array_shift($idx);
					$idx[]=count($idx)+1;
       ...
       foreach ($resizearray as $dimension)    {
               ...
               $twidth=$dimension; $theight=$dimension;
       ...
       //get resize ratio
       if ($heightratio<$widthratio) { $resizeratio=$heightratio; }
...
function rstring ($rs) {
       ...
       $rs = preg_replace("/\ /", "", $rs);
       if (! isuid($rs)) { return false; }

       $rip = preg_replace ("/\./", "", $_SERVER['REMOTE_ADDR']);
       if (! isnumber($rip)) { return false; }

       $site = preg_replace ("/\./", "", $_SERVER['HTTP_HOST']);
       if (! isuid($site)) { return false; }

       $datething = date ("iWsG");
       if (! isnumber($datething)) { return false; }
...
function productswithimages ($checkboxname, $colcount, $array, $field, $selectedids,
$imagesar) {
...
function buttonsdiv ($colcount, $array) {
 ...
 foreach ($array as $key=>$page) {
   if ($i==0) {
     ...
     if ($i % $colcount == 0) { echo "</div><div>ben";}
   }?>
   ...
   $style='';
		if ((int)$product['imageid']>0) 
			if (array_key_exists($product['imageid'], $images)) {
				...
				echo "<table cellpadding=0 border=0 cellspacing=0>";
				echo "<tr><td>";?>
				<a href='http://<?=$_SERVER['HTTP_HOST'].$site->dirs->path($did);?>'><?="<img onclick=\"alert (getimgsize('img".$product['imageid']."'));\" id='img".$product['imageid']."' src='/".IPATH."/".$images[$product['imageid']]['path']."/$version/".$images[$product['imageid']]['filename']."'></a></td></tr>";
...
/*
abstract class site extends mysql {
 private $directories='';
 private $pagename='';
 private $scripturl='scripturl';
 abstract public function setup($f = '');
}
 */
?>
