...
class lib_model_mysql { 
	...
	final protected function setsql($afields = array(), $avalues=array()) {
		...
		if (! is_array ($afields)) { return false; } if (empty ($afields)) { return false; } 
		if (! is_array ($avalues)) { return false; } if (empty ($avalues)) { return false; }
		...
		foreach ($afields as $key=>$type)  {
			...
			$s = ""; $s=mysql_escape_string($avalues[$key]);
	...
	public function connect($mysqlconf = false) {
		...
		$this->ffhost=$lar['dbhost']; $this->ffdb=$lar['dbname'];
		$this->ffuid=$lar['dbuser']; $this->ffpwd=$lar['dbpwd'];
	...
	final public function update($atablename='', $afields=array(), $avalues=array(), $where='') {
		...
		if (! $this->selectdb()) { return (boolean) false; }
		if (! $this->executesql()) { return (boolean) false; }
    ...
    final public function row($s = '', $values=array()) {
			...
			foreach ($values as &$value) {
				if (preg_match("/^([A-Z]|[0-9]|\.|,|\+|\@|\-|\_|~|\.)+$/i", $value)) continue;
				return false;
			}
	  	...
	  	if ((int)mysql_num_rows($arow)===1) {
	    	...
	    	if (! is_array($er)) { return false; }
				if (empty($er)) { return false; }
				if (! isset($er)) { return false; }
				return (array) $er;
			}
